...
float vender(float precio, float total, float total_a_pagar)
{
    ...
    strcpy(Facturas->nombre_producto,"%s" ); //,"Aca va lo que vas a guardar."
    strcpy(Facturas->Cantidad,"%d"/*Aqui lo que hara es introducir la cantidad de productos*/ );
    // TODO La guardare en alguna funcion de inventario que reste los productos
    
    //Precio no se de donde va a salir, creo que de inventario
...
{ //Da la opcion de agregar mas articulos a la factura con 1, sino imprime el total a pagar
    int i, j;
    ...
    do
    {
        ...
        {
        case 1:
            float vender(precio, total, total_a_pagar); //como puedo llamar la funcion aqui para que se ejecute en este caso
            break;
